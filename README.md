...
| `subnet-id`                                                                                                                                                                  | Required if you use the `start` mode.      | One or more VPC Subnet Ids, separated by commas or whitespace. <br><br> All subnets must belong to the same VPC as the specified security group. Subnets in several availability zones widen the spot pool choice and lower the interruption probability.                                                                             |
...
| `market-type` | Optional. Used only with the `start` mode. | Valid options are `spot` and `on-demand`. If `spot` is specified, a Spot instance will be requested. If `on-demand` is specified or the input is left unspecified, an on-demand instance will be provisioned. |
